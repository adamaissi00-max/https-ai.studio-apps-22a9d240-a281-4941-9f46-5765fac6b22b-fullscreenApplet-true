...
import { PRODUCTS, CATEGORIES, STORE_NAME } from './data/products';
import { CartItem, Product } from './types';
import { SearchX, ShoppingBag, Sparkles } from 'lucide-react';

const CART_STORAGE_KEY = 'saadi_grocery_cart_v1';
    ...
    <div className="min-h-screen flex flex-col bg-[#f8faf9] text-gray-800">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 pb-24 sm:pb-12">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {searchQuery && (
            <p className="text-xs text-gray-500">
              نتائج البحث عن: &quot;<span className="font-bold text-gray-800">{searchQuery}</span>&quot;
            </p>
          )}
        </div>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-200/80 p-8">
            ...
            <p className="text-xs sm:text-sm text-gray-500 max-w-sm mx-auto mb-4">
              تأكد من كتابة اسم المنتج بشكل صحيح أو تصفح الفئات المتاحة في المتجر.
            </p>
            <button
              onClick={() => {
                setSearchQuery('');
